...
class DrawView(QGraphicsView):
    ...
    def mousePressEvent(self, e):
        ...
        self._flood_mask[:] = 0
        cv.floodFill(self.img, self._flood_mask, (e.x(), e.y()), 0, self.threshold, self.threshold,
    ...
    def image_save(self):
        ...
        i = 0
        while True:
            # jpg 로 석택된 Threshold 이미지 경로
            file_name = f'mask_{i}.jpg'
            file_path = f'{IMAGE_SAVE_PATH}{file_name}'

            # 같은 파일이 있으면 i 카운트 + 1
            if os.path.isfile(file_path):
                i = i + 1
            else:
                # 같은 파일이 없으면 Threshold 이미지 저장
                cv_path = f'{IMAGE_SAVE_PATH}{file_name}'
                cv.imwrite(cv_path, self.mask)
                break
